#!/usr/bin/env python3
"""Import binary assets from immutable text manifests.

Each manifest is an independent job. A rejected manifest gets its own receipt and
never prevents unrelated new manifests from being attempted.
"""
...
def download_url(url: str, limit: int) -> bytes:
    ...
    req = Request(url, headers={"User-Agent": "alisa-instagram-asset-import/2.1"})
    chunks: list[bytes] = []
    total = 0
...
def validate_magic(data: bytes, suffix: str) -> None:
    ...
    ok = (
        ...
        or (suffix in {".mp4", ".mov"} and data[4:8] == b"ftyp")
    )
    if not ok:
...
def source_label(obj: dict) -> str:
    if obj.get("firestorage_share_url"):
        return "verified firestorage.ai public share (signed raw URL omitted)"
    if obj.get("source_url"):
        parsed = urlparse(obj["source_url"])
        return f"https://{parsed.hostname}{parsed.path} (query omitted)"
...
def process(manifest: Path) -> bool:
    ...
    modes = sum(bool(obj.get(key)) for key in ("source_url", "base64", "base64_chunks", "firestorage_share_url"))
    if modes != 1:
        fail("manifest must contain exactly one binary source mode")

    if obj.get("firestorage_share_url"):
        filename = obj.get("expected_filename")
        expected_bytes = obj.get("expected_bytes")
        if not isinstance(filename, str) or not filename:
            fail("firestorage source requires expected_filename")
        if not isinstance(expected_bytes, int) or expected_bytes < 1:
            fail("firestorage source requires positive expected_bytes")
        data = download_share_file(
            str(obj["firestorage_share_url"]),
    ...
    expected_sha = str(obj.get("expected_sha256", "")).strip().lower()
    ...
    receipt.write_text("\n".join(lines) + "\n", encoding="utf-8")
    print(f"IMPORTED {target.relative_to(ROOT)} bytes={len(data)} sha256={actual_sha}")
    return True
