...
Normal autonomous agent proposals can never modify either the canonical image or identity metadata. Repair of corrupt canonical bytes is a separate guarded maintenance operation and must preserve independently verified identity/checksum policy.
...
2. repository importer from a verified public-share source that resolves to exact bytes and is locked by expected SHA/size/decode checks;
3. `production/import-queue/` via base64 chunks for manageable files;
4. unsigned public direct HTTPS URL;
5. Gmail only as product-to-product byte transport, never as coordination state.

A queue manifest is immutable and independent. One failed manifest must not poison other imports. Do not claim upload success until the target and receipt exist on `main` with matching SHA-256 and required decode/dimension checks.
